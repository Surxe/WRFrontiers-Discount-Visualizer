...
1. **[Backend Documentation](./backend/design_doc.md)**: Details the Python pipeline for compiling game data, mapping items with Jev (TypeSafe AI's typed closed-set classifier), storing mappings in a persistent manual override file, and generating visual grid outputs.
...
3. **Step 2**: The backend maps the input item names against `game_data.json` and a persistent `manual_mapping.json`, resolving in order: manual pin -> exact vocab match -> Jev (TypeSafe AI's typed closed-set classifier). Jev is the mapper: without a `JEV_API_KEY` the step fails fast rather than guessing. Non-1:1 matches are saved to `manual_mapping.json` to respect future overrides. Virtual Bots are expanded to their core modules, filtering out titan weapons. Mapped IDs are output to `temp/output/discounts.json`.
